...
- **Retry com backoff exponencial para 429/503 do Gemini** (`app/infra/llm_client.py`, via `tenacity`, até 5 tentativas). Mesmo assim, o free tier tem cota baixa por minuto — perguntas pesadas em sequência rápida (ex: pergunta 5, que dispara `summarize` para os 5 papers) ainda podem esgotar a cota do dia caso o retry não seja suficiente.
